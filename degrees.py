...
def shortest_path(source, target):
    ...
    # TODO
    initNeighbors = neighbors_for_person(source)
    ...
    while not PathFound:
        ...
        # grab node from our frontier
        thisNode = frontier.remove()

        # check if goal node
        if(thisNode.state[1] == target):
            return returnSolution(thisNode)

        explored.append(thisNode.state)

        # add neighbors to the frontier
        thisNeighbors = neighbors_for_person(thisNode.state[1])
